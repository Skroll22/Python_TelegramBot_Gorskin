...
from secrets_data import API_TOKEN
import datetime
...
def check_user_availability_sync(telegram_id: int, date: date,
    ...
    try:
        ...
        # Проверяем события на весь день
        ...
        # Проверяем пересечения с собственными событиями, у которых есть время
        overlapping_own_meetings = Meeting.objects.filter(
            organizer=user,
            date=date
        ).filter(
            models.Q(
        ...
        return not (overlapping_meetings or overlapping_own_meetings)
...
async def create_meeting_date_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['meeting_date'] = date_str
...
async def create_meeting_description_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Доступное время: с 08:00 до 22:00"
    )
...
async def create_meeting_start_time_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if start_time < time(8, 0) or start_time > time(22, 0):
            await update.message.reply_text(
                "❌ Время должно быть с 08:00 до 22:00.\n"
                "Введите время начала встречи:"
            )
            return CREATE_MEETING_START_TIME

        context.user_data['meeting_start_time'] = start_time_str
    except ValueError:
        await update.message.reply_text(
...
async def create_meeting_end_time_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Проверяем корректность времени
        end_time = datetime.strptime(end_time_str, "%H:%M").time()
        start_time = datetime.strptime(context.user_data['meeting_start_time'], "%H:%M").time()

        # Проверяем, что время окончания позже времени начала
        if end_time <= start_time:
            await update.message.reply_text(
                "❌ Время окончания должно быть позже времени начала.\n"
                "Введите время окончания встречи:"
            )
            return CREATE_MEETING_END_TIME

        # Проверяем минимальную длительность (30 минут)
        start_datetime = datetime.combine(date.today(), start_time)
        end_datetime = datetime.combine(date.today(), end_time)
        duration = (end_datetime - start_datetime).total_seconds() / 60  # в минутах

        if duration < 30:
            await update.message.reply_text(
                "❌ Минимальная длительность встречи - 30 минут.\n"
                "Введите время окончания встречи:"
            )
            return CREATE_MEETING_END_TIME

        # Проверяем максимальную длительность (8 часов)
        if duration > 480:
            await update.message.reply_text(
                "❌ Максимальная длительность встречи - 8 часов.\n"
                "Введите время окончания встречи:"
            )
            return CREATE_MEETING_END_TIME

        context.user_data['meeting_end_time'] = end_time_str
    ...
    await update.message.reply_text(
        ...
        "💡 Узнать ID пользователя можно командой /my_id"
    )
    return CREATE_MEETING_PARTICIPANTS


async def show_meeting_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показать подтверждение создания встречи"""
    date_str = context.user_data['meeting_date']
    title = context.user_data['meeting_title']
    start_time = context.user_data['meeting_start_time']
    end_time = context.user_data['meeting_end_time']
    ...
    if context.user_data.get('meeting_description'):
        desc = context.user_data['meeting_description']
        if len(desc) > 100:
            desc = desc[:97] + "..."
        confirmation_text += f"📝 Описание: {desc}\n"

    participant_ids = context.user_data.get('meeting_participant_ids', [])

    if participant_ids:
        # Получаем информацию об участниках для отображения
        def get_participants_info(p_ids):
            participants = []
            for pid in p_ids:
                try:
                    user = TelegramUser.objects.get(telegram_id=pid)
                    name = user.first_name or user.username or f"ID: {user.telegram_id}"
                    participants.append(name)
                except:
                    participants.append(f"ID: {pid}")
            return participants

        participants_info = await asyncio.to_thread(get_participants_info, participant_ids)
        participants_list = "\n".join([f"  • {info}" for info in participants_info])

        confirmation_text += f"👥 Участники ({len(participant_ids)}):\n{participants_list}\n"
    else:
        confirmation_text += "👤 Только для вас (без дополнительных участников)\n"

    confirmation_text += "\n✅ Для создания введите 'да'\n❌ Для отмены введите 'нет'"

    # Сохраняем флаг ожидания подтверждения
...
async def create_meeting_participants_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('awaiting_confirmation'):
        return await create_meeting_confirmation_handler(update, context)
    ...
    try:
        if input_text == '0':
            participant_ids = []
            context.user_data['meeting_participant_ids'] = participant_ids
            await show_meeting_confirmation(update, context)
            return CREATE_MEETING_PARTICIPANTS
        ...
        for pid in input_text.split(','):
            try:
                pid_clean = pid.strip()
                if not pid_clean:
                    continue

                participant_id = int(pid_clean)

                # Проверяем, что это не ID организатора
                if participant_id == user_id:
                    await update.message.reply_text(
                        f"⚠️  Вы указали свой ID ({user_id}). "
                        f"Организатор добавляется автоматически."
                    )
                    continue

                # ПРОВЕРЯЕМ РЕГИСТРАЦИЮ ПОЛЬЗОВАТЕЛЯ
                def check_and_get_user(pid):
                    try:
                        user = TelegramUser.objects.get(telegram_id=pid)
                        return user, True
                    except TelegramUser.DoesNotExist:
                ...
                user, user_exists = await asyncio.to_thread(check_and_get_user, participant_id)

                if user_exists and user:
                    participant_ids.append(participant_id)

                    # Сохраняем информацию о пользователе для отображения
                    user_info = f"{user.first_name or user.username or f'ID: {user.telegram_id}'}"
                    valid_participants_info.append(user_info)

                    # Логируем
                    logger.info(f"Найден участник: {user.telegram_id} - {user_info}")
                else:
                    invalid_ids.append(pid_clean)
                    logger.warning(f"Пользователь с ID {participant_id} не зарегистрирован в системе")

            except ValueError:
                invalid_ids.append(pid)

        # Сообщаем о найденных пользователях
        if valid_participants_info:
            participants_text = "✅ Найдены участники:\n"
            for i, info in enumerate(valid_participants_info, 1):
                participants_text += f"{i}. {info}\n"
            await update.message.reply_text(participants_text)

        # Сообщаем о неверных ID
        if invalid_ids:
            invalid_msg = (
                f"⚠️  Следующие ID некорректны или пользователи не зарегистрированы: {', '.join(invalid_ids)}\n\n"
                f"💡 Чтобы пользователь мог участвовать во встречах, ему нужно:\n"
                f"1. Запустить этого бота командой /start\n"
                f"2. Узнать свой ID командой /my_id\n"
                f"3. Сообщить этот ID вам"
            )
            await update.message.reply_text(invalid_msg)

        # Если все ID невалидны и нет валидных участников
        if invalid_ids and not participant_ids:
            await update.message.reply_text(
                "❌ Не указано ни одного валидного ID участника.\n"
                "Введите ID участников через запятую или 0:"
            )
            return CREATE_MEETING_PARTICIPANTS
        ...
        logger.error(f"Ошибка при обработке участников: {e}")
        await update.message.reply_text(
...
async def create_meeting_confirmation_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка подтверждения создания встречи"""
    user_id = update.effective_user.id
    response = update.message.text.lower()

    # Список подтверждающих ответов
    confirm_responses = ['да', 'yes', 'y', 'д', 'ок', 'ok', 'создать', 'create']
    cancel_responses = ['нет', 'no', 'n', 'н', 'отмена', 'cancel']

    if response not in confirm_responses + cancel_responses:
        await update.message.reply_text(
            "❌ Пожалуйста, ответьте 'да' или 'нет'.\n"
            "✅ Для создания введите 'да'\n❌ Для отмены введите 'нет'"
        )
    ...
    if response in cancel_responses:
        ...
        # Очищаем данные
        meeting_keys = ['meeting_date', 'meeting_title', 'meeting_description',
                        'meeting_start_time', 'meeting_end_time', 'meeting_participant_ids',
    ...
    # Создаем встречу
    try:
        # Показываем пользователю, что идет создание
        ...
        # Показываем детали созданной встречи
        date_str = context.user_data['meeting_date']
        start_time = context.user_data['meeting_start_time']
        end_time = context.user_data['meeting_end_time']
        title = context.user_data['meeting_title']
        ...
        meeting_details = (
            ...
            f"• 📅 Дата: {date_str}\n"
            f"• 🕐 Время: {start_time} - {end_time}\n"
            f"• 🆔 ID встречи: {meeting.id if meeting else 'N/A'}\n"
        )

        if context.user_data.get('meeting_description'):
            meeting_details += f"• 📝 Описание: {context.user_data['meeting_description']}\n"

        participant_ids = context.user_data.get('meeting_participant_ids', [])
        if participant_ids:
            meeting_details += f"• 👥 Участников: {len(participant_ids)}\n"
        else:
            meeting_details += "• 👤 Только для вас\n"

        meeting_details += (
            f"\n💡 Команды для управления:\n"
            f"• Просмотр: /meeting {meeting.id if meeting else ''}\n"
            f"• Все встречи: /meetings\n"
            f"• Приглашения: /invitations\n"
        )

        result += meeting_details

        # Отправляем приглашения участникам
        if meeting and hasattr(meeting, 'id'):
            try:
                # Получаем участников, ожидающих приглашений
                def get_pending_participants():
                    pending = MeetingParticipant.objects.filter(
                        meeting=meeting,
                        status='pending'
                ...
                if pending_participants:
                    ...
                    for mp in pending_participants:
                        try:
                            invitation_message = (
                                ...
                                f"📅 Дата: {date_str}\n"
                                f"🕐 Время: {start_time} - {end_time}\n"
                                f"👑 Организатор: {update.effective_user.first_name or update.effective_user.username}\n"
                            )

                            if context.user_data.get('meeting_description'):
                                invitation_message += f"📝 Описание: {context.user_data['meeting_description']}\n"

                            invitation_message += (
                                f"\n✅ Подтвердить: `/confirm_meeting_{meeting.id}`\n"
                                f"❌ Отклонить: `/decline_meeting_{meeting.id}`\n\n"
                                f"📋 Все приглашения: /invitations"
                            )

                            await send_telegram_notification_to_user(
                                context,
                                mp.participant.telegram_id,
                                invitation_message
                            )

                            logger.info(f"Приглашение отправлено пользователю {mp.participant.telegram_id}")

                        except Exception as e:
                            logger.error(f"Ошибка при отправке приглашения {mp.participant.telegram_id}: {e}")

                    result += f"\n\n📨 Приглашения отправлены {len(pending_participants)} участникам!"

            except Exception as e:
                logger.error(f"Ошибка при отправке приглашений: {e}")
                result += f"\n\n⚠️ Приглашения сохранены в системе, но возникла ошибка при отправке."

    except Exception as e:
        logger.error(f"Критическая ошибка при создании встречи: {e}")
        result = f"❌ Критическая ошибка при создании встречи: {str(e)}"
    ...
    create_meeting_conv_handler = ConversationHandler(
        ...
        states={
            ...
            CREATE_MEETING_PARTICIPANTS: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, create_meeting_participants_handler)],
        },
        fallbacks=[CommandHandler('cancel', cancel_handler)]
    )
